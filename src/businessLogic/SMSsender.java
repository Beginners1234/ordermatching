...
public class SMSsender {
	...
	public void test() {
		...
		sendSMStoUsers(order1,order2,new Trade());
		
	}
	public void sendSMStoUsers(Order order1,Order order2,Trade trade) {
		...
		String message1="Your "+order1.getOrderCategory()+" " +order1.getOrderType()+
					" order of Quantity "+order1.getOrderQuantity()+" was executed at "+trade.getTradedPrice()
					+" Status is now: "+order1.getOrderStatus()+" Reamaining Qty: "
					+order1.getRemaining_quantity();
		String message2="Your"+order2.getOrderCategory() +order2.getOrderType()+
				"order of Quantity"+order2.getOrderQuantity()+" was executed at "+trade.getTradedPrice()
	...
	//	sendsms(contact1,message1);
	 ...
	 private void sendsms(String recipient,String message) {
         try {
        	 	...
        	 	if(recipient.length()!=10)throw new Exception();
